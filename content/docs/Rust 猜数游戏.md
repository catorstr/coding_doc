...
匹配表达式由手臂组成。一个arm由一个要匹配的模式和一段代码组成，如果要匹配的值符合该arm的模式则应该运行该代码。Rust接受给定的匹配值，并依次查看每个手臂的图案。模式和match结构是强大的Rust特性:它们允许您表达代码可能遇到的各种情况，并确保您能够处理所有这些情况。些特性将分别在第6章和第18章中详细介绍。
